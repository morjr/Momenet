...
def get_model(point_cloud, is_training, bn_decay=None):
    ...
    edge_feature_mor = tf_util.get_edge_feature_mor2(point_cloud, k=3)

    edge_feature =tf.concat([edge_feature, edge_feature_mor], axis=-1)
    net_p = edge_feature
    ...
    nn_idx = tf_util.knn(adj_matrix, k=20)
    edge_feature = tf_util.get_edge_feature(point_cloud_transformed, nn_idx=nn_idx, k=20)
    edge_feature_mor = tf_util.get_edge_feature_mor2(point_cloud_transformed, k=3)
    ...
    net_= tf.reduce_max(net, axis=-2, keep_dims=True)
    net1 = net_

    #net = tf.concat([net_p, net0, net], axis=-1)
