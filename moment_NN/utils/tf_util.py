...
def get_edge_feature_mor(point_cloud, k=64):
    """Construct edge feature for each point
    Args:
      point_cloud: (batch_size, num_points, 1, num_dims)
      nn_idx: (batch_size, num_points, k)
    ...
    point_cloud_central = tf.expand_dims(point_cloud_central, axis=-2)
    rolled_point_cloud = point_cloud_central
    # rolled_point_cloud = tf.manip.roll(point_cloud_central, shift=1, axis=-1)
    tmp_point_momentum = tf.multiply(point_cloud_central, rolled_point_cloud)
    point_cloud_momentum = tmp_point_momentum
    rolled_point_cloud = tf.manip.roll(rolled_point_cloud, shift=1, axis=-1)
    for i in range(k - 1):
        tmp_point_momentum = tf.multiply(point_cloud_central, rolled_point_cloud)
        point_cloud_momentum = tf.concat([point_cloud_momentum, tmp_point_momentum], axis=-2)
        rolled_point_cloud = tf.manip.roll(rolled_point_cloud, shift=1, axis=-1)

    rolled_point_cloud_square = point_cloud_central
    rolled_point_cloud_square = tf.multiply(rolled_point_cloud_square, rolled_point_cloud_square)
    # rolled_point_cloud = tf.manip.roll(point_cloud_central, shift=1, axis=-1)
    tmp_point_momentum = tf.multiply(point_cloud_central, rolled_point_cloud_square)
    point_cloud_momentum_square = tmp_point_momentum
    rolled_point_cloud = tf.manip.roll(rolled_point_cloud_square, shift=1, axis=-1)
    for i in range(k - 1):
        tmp_point_momentum = tf.multiply(point_cloud_central, rolled_point_cloud)
        point_cloud_momentum_square = tf.concat([point_cloud_momentum_square, tmp_point_momentum], axis=-2)
        rolled_point_cloud = tf.manip.roll(rolled_point_cloud, shift=1, axis=-1)

    point_cloud_central = tf.tile(point_cloud_central, [1, 1, k * k, 1])
    point_cloud_momentum = tf.reshape(tf.tile(point_cloud_momentum, [1, 1, 1, k]), [batch_size, num_points, k * k, -1])
    point_cloud_momentum_square = tf.tile(point_cloud_momentum_square, [1, 1, k, 1])
    momentum_feature = tf.concat([point_cloud_central, point_cloud_momentum, point_cloud_momentum_square], axis=-1)
    return momentum_feature


def get_edge_feature_mor2(point_cloud, k=64):
    """Construct edge feature for each point
    Args:
      point_cloud: (batch_size, num_points, 1, num_dims)
      nn_idx: (batch_size, num_points, k)
    ...
    #point_cloud_central = tf.tile(point_cloud_central, [1, 1, k * k, 1])
    point_cloud_momentum = tf.reshape(tf.concat([point_cloud_momentum,point_cloud_momentum_square],axis=-1), [batch_size,num_points,1,-1])
    point_cloud_momentum = tf.tile(point_cloud_momentum, [1,1,20,1],)
    #point_cloud_momentum_square = tf.tile(point_cloud_momentum_square, [1, 1, k, 1])
    #momentum_feature = tf.concat([point_cloud_central, point_cloud_momentum, point_cloud_momentum_square], axis=-1)
    return point_cloud_momentum
